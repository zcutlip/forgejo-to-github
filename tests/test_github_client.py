...
def test_rate_limit_sleep_capped_on_far_future_reset() -> None:
    """A far-future Retry-After is capped at _MAX_RATE_LIMIT_SLEEP so
    the migration does not sleep for hours. The sleep must not exceed
    _MAX_RATE_LIMIT_SLEEP + _JITTER_SECONDS (61.0s).
    """
    from forgejo_to_github.github import _JITTER_SECONDS, _MAX_RATE_LIMIT_SLEEP
