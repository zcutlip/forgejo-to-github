...
| 8 | Each public class has 2–9 public methods | `tests/test_package_boundaries.py::test_public_class_has_at_least_two_public_methods` and `test_public_class_has_at_most_nine_public_methods` pass for all six classes | test framework spec §14.5 (cap amended from seven to nine during audit remediation; see issue #7) |
...
| 21 | Dry-run exits 0, issues read-only GET requests only, invokes no subprocess, writes no state, reports the discovered issue count via `issues_discovered` while keeping `issues_attempted == 0`, carries a populated `DryRunDiscovery` value (target repo, repo existence, discovered comment count, state path, checkpoint count; `None` on normal runs), and renders the approved dry-run preview summary | `tests/test_orchestration.py::test_dry_run_issues_only_get_requests`, `test_dry_run_makes_no_subprocess_calls`, `test_dry_run_does_not_write_state`, and `test_dry_run_reports_discovered_issue_count` pass (the latter asserting `issues_attempted == 0`, `issues_discovered == 2`, the populated `DryRunDiscovery` value, and "would process 2 issues"); `tests/test_reporting.py::test_dry_run_summary_does_not_claim_migrated` passes (asserting the preview lines, including `Dry-run complete — no changes were made.`, `Repo: would be created`/`Repo: existing`, `Comments: would post M`, and `State: path (K checkpointed)`) | `00-index.md` dry-run decision; stage 04 §3.2 step 1, §3.7; stage 05 §3.4 rule 6; stage 06 §3.3 |
