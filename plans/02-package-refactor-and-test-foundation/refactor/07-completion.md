...
| 21 | Dry-run exits 0, issues read-only GET requests only, invokes no subprocess, writes no state, reports the discovered issue count via `issues_discovered` while keeping `issues_attempted == 0` | `tests/test_orchestration.py::test_dry_run_issues_only_get_requests`, `test_dry_run_makes_no_subprocess_calls`, `test_dry_run_does_not_write_state`, and `test_dry_run_reports_discovered_issue_count` pass (the latter asserting `issues_attempted == 0`, `issues_discovered == 2`, and "would process 2 issues"); `tests/test_reporting.py::test_dry_run_summary_does_not_claim_migrated` passes | `00-index.md` dry-run decision; stage 04 §3.2 step 1; stage 05 §3.4 rule 6 |
